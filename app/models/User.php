...
class User
{
    ...
    public function getRoleByToken(string $token)
    {
        $sql = "SELECT u.role_id FROM password_resets p 
       INNER JOIN users u ON p.user_id = u.id WHERE p.token = ?";

        $stmt = $this->db->prepare($sql);
        $stmt->execute([$token]);
        return $role_id = $stmt->fetchColumn();
    }
